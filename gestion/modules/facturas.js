...
function factGenerarPDF(f, descargar = true) {
  ...
  const W=210,M=18,RED=[253,173,0],DARK=[30,30,30],MUTED=[100,100,100],BORDR=[200,200,200],CREAM=[245,245,245];
  ...
  if(esBeca){
    ...
    const cls=[{txt:'BENEFICIARIO/A BECA NEAE CURSO '+(f._anio_escolar||'')+': '+pN,b:true,i:true},{txt:'PADRE/MADRE O TUTOR/A LEGAL: '+tN,b:true,i:true},{txt:f.receptor_dni?'D.N.I.:'+f.receptor_dni:'',b:false,i:true},{txt:f.receptor_direccion||'',b:false,i:false},{txt:f.receptor_cp?'C.P./'+f.receptor_cp:'',b:false,i:false},{txt:f.receptor_municipio?(f.receptor_municipio==='Granada'?'Granada':f.receptor_municipio+' - Granada'):'',b:false,i:false}].filter(l=>l.txt);
    const ch=Math.max(24,cls.length*4.6+6);doc.setDrawColor(...BORDR);doc.setLineWidth(0.3);doc.rect(M,y,W-M*2,ch);
    let cy=y+6.5;cls.forEach(l=>{doc.setFont('helvetica',l.b?(l.i?'bolditalic':'bold'):(l.i?'italic':'normal'));doc.setFontSize(8.5);doc.setTextColor(...DARK);doc.splitTextToSize(l.txt,W-M*2-8).forEach(ln=>{doc.text(ln,M+4,cy);cy+=4.6;});});y+=ch+2;
  }else{
  ...
  doc.text('CONCEPTO',C1,y+5.5);doc.text('Nº DE HORAS',C2,y+3,{align:'center'});doc.text('TOTALES',C2,y+7,{align:'center'});doc.text('PRECIO HORA',C3,y+5.5,{align:'center'});doc.text('PRECIO TOTAL',C4,y+5.5,{align:'right'});y+=8;
  const lineas=Array.isArray(f.lineas)?f.lineas:(typeof f.lineas==='string'?JSON.parse(f.lineas||'[]'):[]);
  if(esBeca){
    lineas.forEach((l,i)=>{
      const subs=(l.concepto||'').split('\n').filter(Boolean);let tl=0;subs.forEach(s=>{tl+=doc.splitTextToSize(s,C2-C1-4).length;});
      const rh=Math.max(18,tl*4.0+7);
      ...
      let cy=y+5.5;subs.forEach((s,si)=>{doc.setFont('helvetica',si===0?'bolditalic':'normal');doc.setFontSize(8.5);doc.setTextColor(...DARK);doc.splitTextToSize(s,C2-C1-4).forEach(ln=>{doc.text(ln,C1,cy);cy+=4.0;});});
  ...
  [['Base imponible',fmtI(f.base_imponible)+' €',false],['I.V.A.',fmtI(f.iva_importe||0)+' €',true],['Retención Aplicable I.R.P.F',fmtI(f.irpf_importe||0)+' €',true]].forEach(([label,val,italic],i)=>{const rh=6.5;doc.setFillColor(i%2===0?255:248,i%2===0?255:248,i%2===0?255:248);doc.rect(M,y,W-M*2,rh,'F');doc.setDrawColor(...BORDR);doc.setLineWidth(0.2);doc.line(M,y+rh,W-M,y+rh);doc.setFont('helvetica',italic?'bolditalic':'bold');doc.setFontSize(9);doc.setTextColor(...DARK);doc.text(label,M+4,y+4.1);doc.setFont('helvetica',italic?'italic':'normal');doc.text(val,W-M-4,y+4.1,{align:'right'});y+=rh;});
  doc.setFillColor(255,255,255);doc.rect(M,y,W-M*2,9,'F');doc.setDrawColor(...BORDR);doc.rect(M,y,W-M*2,9);doc.setFont('helvetica','bold');doc.setFontSize(10);doc.setTextColor(...DARK);doc.text('TOTAL FACTURA',M+4,y+5.6);doc.text(fmtI(f.total)+' €',W-M-4,y+5.6,{align:'right'});y+=9;
